...
async function handler(req: NextApiRequest, res: NextApiResponse) {
    try {
        ...
        const customer = await axios.post(`${identityUrl}/customers`,
            JSON.stringify({
                "name": req.body.BusinessLegalName
            }), config);

        const insured = await axios.post(`${lulaApi}/marketplace/api/insured`, {
            "Id": customer.data.id,
        ...
        try {
            ...
            errorMessages.push("Failed to add auto liability coverage: " + (error as Error).message);;
        }
        ...
        try {
            ...
            errorMessages.push("Failed to add physical damage coverage: " + (error as Error).message);;
        }
        ...
        try {
            ...
            errorMessages.push("Failed to add cargo coverage: " + (error as Error).message);
        }
        ...
        try {
            ...
            errorMessages.push("Failed to add general liability coverage: " + (error as Error).message);
        }

        if (errorMessages.length > 0) {
            res.status(500).json({ errorMessages });
        } else {
            res.status(200).json(`Added coverages successfully: {${added.join(", ")}}`);
        }
    } catch (error) {
