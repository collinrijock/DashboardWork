...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    if (response.ok) {
      ...
      res.status(response.status).json({ message: errorData || 'Failed to submit the policy. Please try again.' });
    }
  } catch (error: any) {
    console.error('Error submitting policy:', error);
    res.status(500).json({ message: 'An error occurred while submitting the policy. Please try again.', error: error.message });
  }
}
